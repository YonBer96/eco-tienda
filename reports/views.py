...
def _supplier_email_initial(ciclo, proveedor):
    lineas = list(
        LineaPedido.objects
        .filter(pedido__ciclo=ciclo, pedido__estado=Pedido.CONFIRMADO, proveedor_snapshot=proveedor, cantidad__gt=0)
        .values('nombre_producto_snapshot', 'unidad_medida_snapshot')
        .annotate(total_cantidad=Sum('cantidad'))
        .order_by('nombre_producto_snapshot')
    )
...
def enviar_albaran_email(request, pedido_id):
        ...
        if form.is_valid():
            ...
            if attach_pdf:
                try:
                   ...
                   albaran_pdf = render_pdf_bytes('reports/albaran_individual.html', context)
                   factura_pdf = render_pdf_bytes('reports/factura.html', context)

                   if albaran_pdf is None or factura_pdf is None:
                        messages.error(request, 'No se pudieron generar los PDFs del albarán y la factura.')
                        return redirect('pedido_detail', pk=pedido.id)
                   ...
                   email.attach(
                        f'factura_pedido_{pedido.id}.pdf',
                        factura_pdf,
                        'application/pdf'
                    )
    ...
    return render(request, 'reports/email_draft.html', {
        ...
        'summary_lines': [
            f'{linea.nombre_producto_snapshot} · {linea.cantidad:.2f} {linea.unidad_medida_snapshot} · {linea.total_con_iva:.2f} €'
            for linea in pedido.lineas.filter(cantidad__gt=0)
        ],
